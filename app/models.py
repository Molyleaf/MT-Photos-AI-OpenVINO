...
class AIModels:
        ...
        try:
            ...
            if self.face_pool.empty():
                ...
                for i in range(INFERENCE_WORKERS):
                    logging.info(f"加载 FaceAnalysis 实例 {i+1}/{INFERENCE_WORKERS}...")
                ...
                logging.info(f"--- {INFERENCE_WORKERS} 个工作实例已准备就绪 ---")
            else:
                logging.info("--- 工作实例池已填充，跳过填充。 ---")

            self.models_loaded = True
            logging.info("--- 所有模型已成功加载并编译 ---")
        except Exception as e:
            logging.critical(f"模型加载失败: {e}", exc_info=True)
            self.release_models() # 出错时尝试清理
            raise
    ...
    def _load_insightface(self) -> FaceAnalysis:
        try:
            ...
            logging.debug("InsightFace 实例加载成功。")
            return face_app
        except Exception as e:
            logging.error(f"加载 InsightFace 实例失败: {e}", exc_info=True)
            raise
    ...
    def _load_rapidocr(self) -> RapidOCR:
        try:
            ...
            ocr = RapidOCR()
            logging.debug("RapidOCR 实例加载成功。")
            return ocr
        except Exception as e:
            logging.error(f"加载 RapidOCR 实例失败: {e}", exc_info=True)
            raise
    ...
    def _load_qa_clip(self):
        ...
        try:
            ...
            logging.info(f"QA-CLIP ({MODEL_ARCH}) 基础模型编译成功。")
