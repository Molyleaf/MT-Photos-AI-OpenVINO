...
def restore_model(model, clip_state_dict: dict, bert_state_dict: dict = None):
    merged_state_dict = {}
    ...
    # use bert_state_dict to initialize the text encoder (optional, usually inside clip_state_dict now)
    if bert_state_dict is not None:
        for k, v in bert_state_dict.items():
            if k.startswith("bert") and "bert.pooler" not in k:
                merged_state_dict[k] = v
    elif clip_state_dict is not None:
        # Fallback: try finding bert in clip_state_dict
        for k, v in clip_state_dict.items():
            if k.startswith("bert") and "bert.pooler" not in k:
                merged_state_dict[k] = v
