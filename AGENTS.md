...
>
> 主要痛点（必须被解决）：**随意改接口语义、随意替换推理后端、改坏 QA-CLIP 子库、线程/进程过度并发导致抖动、模型重复驻留导致内存暴涨、文档与依赖不一致**。
>
...
- 语言与运行时：**Python 3.12**（目标平台：Windows Server）。
...
- [ ] RapidOCR 是否为 `rapidocr==3.6.0` 且 OpenVINO CPU 后端
