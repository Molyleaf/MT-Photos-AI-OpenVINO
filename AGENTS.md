...
- 父进程非文本模型族切换必须保持请求到达顺序上的公平性：当不同模型族都在等待时，后到请求不得插队抢占先到请求的切换机会；至少要保证 `vision/ocr/face` 三族在父进程调度层是对称的，不会因为同族新请求不断到达而把异族请求长期饿死。
- 必须控制总并行度：
  - `总并行度 = 各阶段执行器线程数之和`
- 原因：
...
- 非文本父进程管理器已修复混合输入死锁：当 `/represent` 尚未完成时，新到达的 `/clip/img` / `/ocr` 请求改为异步等待模型族切换，不再把事件循环卡在 `Condition.wait` 上。
- 父进程模型族准入现已收敛为 FIFO 公平轮换，修复了多等待者竞争下的插队和潜在饥饿；对应回归新增了六种有序两两轮换测试、三族 round-robin 测试和轻量 mixed burst 压测脚本。
